...
public class ClientJetpackHandler {
    ...
    public void showJetpackParticles(World world, PlayerEntity player, JetpackParticleType particleType) {
        ...
        if (minecraft.options.particles != ParticleStatus.MINIMAL && particle != null) {
            ...
            Pos3D vLeft = new Pos3D(-0.28, -0.95, -0.35).rotate(minecraft.player.yBodyRot, 0);
            Pos3D vRight = new Pos3D(0.28, -0.95, -0.35).rotate(minecraft.player.yBodyRot, 0);
            Pos3D vCenter = new Pos3D((rand.nextFloat() - 0.5F) * 0.25F, -0.95, -0.38).rotate(minecraft.player.yBodyRot, 0);
