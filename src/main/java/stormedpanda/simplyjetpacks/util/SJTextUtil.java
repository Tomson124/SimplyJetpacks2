...
/*        if (jetpack.getType().canEHover()) {
        ...
        if (!jetpack.isCreative()) {
            list.add(translate("tooltip", "itemJetpack.energyUsage", SJStylesUtil.GOLD, energyPerTick(jetpack.getEnergyExtract())));
        }
        ITextComponent particle = translate("tooltip", "particle." + JetpackItem.getParticleId(stack), SJStylesUtil.WHITE);
